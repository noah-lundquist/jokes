...
$hashed_pass = password_hash($new_password1, PASSWORD_DEFAULT);
...
if(strlen($new_password1) <= 8) {
    echo 'The password must have at least 8 characters. <br>';
    exit;
}
...
// check to see if the password fields match
    ...
    // add the new user
    $sql = "INSERT INTO users (user_id, user_name, password) VALUES (null, '$new_username', '$hashed_pass')";
    $result = $mysqli->query($sql) or die (mysqli_error($mysqli));
    if ($result) {
